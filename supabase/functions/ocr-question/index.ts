      ...
      messages: [
        {
          role: 'system',
          content:
            '你是 OCR 与题目结构化助手。请尽量完整还原题面，保留换行、编号、选项与子问。优先输出可直接阅读的数学符号（如 × ÷ ≤ ≥），不要输出 \\times。仅在复杂公式（分式、根式、上下标）时使用 LaTeX，并且必须成对使用 $...$ 或 $$...$$。若包含几何图形，补充图中的关键文字、点名、边角关系和已知条件。只输出题目内容，不要解释。'
        },
        {
          role: 'user',
          content: [
            {
              type: 'text',
              text: '请完整识别这张题目图片。重点检查每个选项中的运算符，x/*/· 请还原为 ×。不要漏掉单位、括号、分隔符和小数点。'
            },
            {
              type: 'image_url',
              image_url: {
